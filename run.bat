...
if not defined JAVA_HOME (
    ...
    for /f "usebackq tokens=*" %%i in (`java -XshowSettings:properties -version 2^>^&1 ^| findstr "java.home"`) do (
        set "JAVA_LINE=%%i"
    )
    
    REM Extract the path from "java.home = <path>"
    if defined JAVA_LINE (
        for /f "usebackq tokens=2* delims==" %%a in ('!JAVA_LINE!') do (
            set "JAVA_HOME=%%a"
            REM Trim leading spaces
            for /f "usebackq tokens=*" %%b in ('!JAVA_HOME!') do set "JAVA_HOME=%%b"
        )
    )
    ...
    if not defined JAVA_HOME (
        ...
        if exist "C:\Program Files\Oracle" (
    ...
    if defined JAVA_HOME (
        ...
        REM Verify it's valid by checking for javac.exe
        if not exist "!JAVA_HOME!\bin\javac.exe" (
            echo WARNING: Detected JAVA_HOME does not contain bin\javac.exe
            echo This may be a JRE instead of a JDK
        )
    )
